...
const FIELD_DESCRIPTIONS = {
  ...
  instance:       { id:"UUID", engagementId:"FK", createdAt:"ISO", updatedAt:"ISO", state:"current|desired", layerId:"FK to LAYERS", environmentId:"FK to environment", label:"Display label", vendor:"Vendor name", vendorGroup:"dell|nonDell|custom", criticality:"High|Medium|Low — criticality LEVEL of this asset/workload (NOT a rank)", notes:"Free-text", disposition:"FK to DISPOSITION_ACTIONS", endOfSaleDate:"Nullable YYYY-MM-DD; UI surfaces it only on compute/storage/dataProtection layers", endOfSupportDate:"Nullable YYYY-MM-DD; feeds lifecycle risk scoring and gap auto-drafting", endOfServiceLifeDate:"Nullable YYYY-MM-DD; past this date a lifecycle risk is 'critical'", nodeCount:"Nullable non-negative integer; node count for the lifecycle-tracked asset", originId:"DESIRED-only: FK to current instance this replaces", priority:"DESIRED-only: Now|Next|Later — phase-of-life, NOT criticality", mappedAssetIds:"WORKLOAD-only: array of FK to instances", aiSuggestedDellMapping:"Provenance-wrapped Dell-mapping suggestion (§S8)", aiTag:"AI-mutation provenance stamp; cleared on next engineer save" },
